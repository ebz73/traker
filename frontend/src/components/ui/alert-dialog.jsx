  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        data-size={size}
        // CUSTOM: max-h-[calc(100vh-2rem)] + overflow-y-auto so the dialog respects short landscape viewports (action buttons stay reachable instead of overflowing off-screen).
        className={cn(
          "group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full max-h-[calc(100vh-2rem)] overflow-y-auto -translate-x-1/2 -translate-y-1/2 gap-3 rounded-xl bg-popover p-4 text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none data-[size=default]:max-w-xs data-[size=sm]:max-w-64 data-[size=default]:sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className
        )}
        {...props} />
    </AlertDialogPortal>
...
function AlertDialogAction({
  ...
  return (
    // Forward className through Button so tailwind-merge can reconcile
    // caller's overrides (e.g. bg-destructive) against Button's variant
    // defaults (bg-primary). Without this, Slot concatenates instead of
    // merging — both classes end up in the DOM and CSS cascade decides
    // arbitrarily. Same fix applied to AlertDialogCancel below.
